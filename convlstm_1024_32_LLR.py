...
def pair_generator_1dconv_lstm(data, labels, start_at=0, generator_batch_size=64, scaled=True, scaler_type='standard',
                               use_precomputed_coeffs=True):  # shape is something like 1, 11520, 11
...
prev_num_sequence_draws = 250  # prev. times trining corpus sampled
generator_batch_size = 1024
generator_batch_size_valid_x1 = (generator_batch_size + 32)  # 4layer conv
generator_batch_size_valid_x2 = (generator_batch_size + 32)
generator_batch_size_valid_x3 = (generator_batch_size + 32)  # 4layer conv
generator_batch_size_valid_x4 = (generator_batch_size + 32)
generator_batch_size_valid_x5 = (generator_batch_size + 32)  # 4layer conv
generator_batch_size_valid_x6 = (generator_batch_size + 32)
generator_batch_size_valid_x7 = (generator_batch_size + 32)  # 4layer conv
generator_batch_size_valid_x8 = (generator_batch_size + 32)
generator_batch_size_valid_x9 = (generator_batch_size + 32)
generator_batch_size_valid_x10 = (generator_batch_size + 32)
generator_batch_size_valid_x11 = (generator_batch_size + 32)
finetune = True
...
save_preds = False
save_figs = False
env = "CHEZ CHAN"  # "cruiser" "chan" #TODO complete the environment_variable_setter
...
# Weights_200_conv1d_samepad_1_32shortrun

# "_conv1d_samepad_" + str(num_epochs) + "_" + str(generator_batch_size) + "shortrun"
# Weights_200_conv1d_samepad_1_32shortrun
identifier_post_training = "BS_" + str(generator_batch_size) + "_GP_32_LLR"
# identifier_pre_training = "_conv1d_samepad_" + str(num_epochs) + "_" + str(generator_batch_size) + "shortrun"
identifier_pre_training = "BS_" + str(generator_batch_size) + "_GP_32_MLR"  # for now, make hardcode what you want to finetune
# @@@@@@@@@@@@@@ RELATIVE PATHS @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
# Base_Path = "./"
# image_path = "./images/"
# train_path = "./train/"
...
# ^^^^^^^^^^^^^ TO RUN ON CHEZ CHAN ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Base_Path = "/home/riset/devin/pitta/convlstm/"
image_path = "/home/riset/devin/pitta/convlstm/img/"
train_path = "/home/riset/devin/pitta/model_ihsan/Train FV1b/"
test_path = "/home/riset/devin/pitta/model_ihsan/Test FV1b"
test_path = "/home/riset/devin/pitta/model_ihsan/FV1b 1d nonlinear/"
...
if finetune == False:
    ...
    weights_present_indicator = os.path.isfile(
        'Weights_' + str(prev_num_sequence_draws) + identifier_pre_training + '.h5')
    print("Are weights (with the given name) to initialize with present? {}".format(weights_present_indicator))

csv_logger = CSVLogger(filename="logtrain_" + identifier_post_training + ".csv", append=True)
...
if (finetune == False and weights_present_indicator == False and test_only == False) or (
    ...
    for i in range(0, num_sequence_draws):
        ...
        if finetune == True:  # load the weights
            finetune_init_weights_filename = 'Weights_' + str(
                prev_num_sequence_draws) + identifier_pre_training + '.h5'  # hardcode the previous epoch number UP ABOVE
