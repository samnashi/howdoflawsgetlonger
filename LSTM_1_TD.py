...
def np_array_pair_generator(data,labels,start_at=0,generator_batch_size=64,scaled=True,scaler_type = 'standard',scale_what = 'data'): #shape is something like 1, 11520, 11
    ...
    while 1:
        ...
        if index + 3 * generator_batch_size < data_scaled.shape[1]:
            ...
            index = np.random.randint(low=0,high=(generator_batch_size*((data_scaled.shape[1]-start_at)//generator_batch_size-3)))
...
if os.path.isfile('Weights_' + str(num_sequence_draws) + identifier + '.h5') == False:
    ...
    for i in range(0,num_sequence_draws):
        ...
        training_hist = model.fit_generator(np_array_pair_generator(train_array,label_array,start_at=0,generator_batch_size=generator_batch_size),epochs=num_epochs,steps_per_epoch=(train_array.shape[0]//generator_batch_size),verbose=2)
...
if os.path.isfile('Weights_' + str(num_sequence_draws) + identifier + '.h5') == True:
    ...
    for files in combined_filenames:
        ...
        generator_starting_index = test_array.shape[
                                       1] - 1 - shortest_length  # steps per epoch is how many times that generator is called
        ...
        row_dict = {}
        score = model.evaluate_generator(test_generator, steps=(1*test_array.shape[0]//generator_batch_size),max_queue_size=test_array.shape[0],use_multiprocessing=True)
